...
MASKGCT_ROOT_PATH = os.environ.get("MASKGCT_ROOT_PATH", "../MaskGCT-models/")
whisper_dir = os.path.join(MASKGCT_ROOT_PATH, "Whisper-large-v3-turbo")
w2v_bert_2_0_dir = os.path.join(MASKGCT_ROOT_PATH, "w2v-bert-2.0")
MaskGCT_dir = os.path.join(MASKGCT_ROOT_PATH, "MaskGCT")


processor = SeamlessM4TFeatureExtractor.from_pretrained(w2v_bert_2_0_dir)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
whisper_model = whisper.load_model(
    "turbo", download_root=whisper_dir, device="cpu")
...
def detect_speech_language(speech_file):
    ...
    if whisper_model == None:
        whisper_model = whisper.load_model(
            "turbo", download_root=whisper_dir)
    ...
    mel = whisper.log_mel_spectrogram(
        audio, n_mels=128).to(whisper_model.device)
...
def build_semantic_model(device):
    semantic_model = Wav2Vec2BertModel.from_pretrained(w2v_bert_2_0_dir)
    semantic_model.eval()
...
def text2semantic(
    ...
    prompt_phone_id = torch.tensor(
        prompt_phone_id, dtype=torch.long).to(device)
    target_phone_id = torch.tensor(
        target_phone_id, dtype=torch.long).to(device)
...
def load_models():
    ...
    semantic_code_ckpt = hf_hub_download(
        "amphion/MaskGCT", filename="semantic_codec/model.safetensors", local_dir=MaskGCT_dir, local_files_only=True
    )
    codec_encoder_ckpt = hf_hub_download(
        "amphion/MaskGCT", filename="acoustic_codec/model.safetensors", local_dir=MaskGCT_dir, local_files_only=True
    )
    codec_decoder_ckpt = hf_hub_download(
        "amphion/MaskGCT", filename="acoustic_codec/model_1.safetensors", local_dir=MaskGCT_dir, local_files_only=True
    )
    t2s_model_ckpt = hf_hub_download(
        "amphion/MaskGCT", filename="t2s_model/model.safetensors", local_dir=MaskGCT_dir, local_files_only=True
    )
    s2a_1layer_ckpt = hf_hub_download(
        "amphion/MaskGCT", filename="s2a_model/s2a_model_1layer/model.safetensors", local_dir=MaskGCT_dir, local_files_only=True
    )
    s2a_full_ckpt = hf_hub_download(
        "amphion/MaskGCT", filename="s2a_model/s2a_model_full/model.safetensors", local_dir=MaskGCT_dir, local_files_only=True
    )
...
def maskgct_inference(
    ...
    acoustic_code = extract_acoustic_code(
...
def inference(
    ...
    save_path = f"./output/output_{int(time.time())}.wav"
